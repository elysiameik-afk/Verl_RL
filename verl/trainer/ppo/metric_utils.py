...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # 🔍 环节4: 检查HVR指标提取
    print(f"🔍4️⃣ [compute_data_metrics] 开始检查non_tensor_batch")
    if hasattr(batch, 'non_tensor_batch') and batch.non_tensor_batch:
        all_keys = list(batch.non_tensor_batch.keys())
        hvr_keys = [k for k in all_keys if 'hvr' in k.lower()]
        print(f"🔍4️⃣ [compute_data_metrics] non_tensor_batch总共 {len(all_keys)} 个字段")
        print(f"🔍4️⃣ [compute_data_metrics] 其中HVR字段 {len(hvr_keys)} 个: {hvr_keys[:5]}...")
        ...
        for key, values in batch.non_tensor_batch.items():
            ...
            if key.startswith('hvr') or 'hvr' in key.lower():
                try:
                    ...
                    print(f"🔍4️⃣    处理 {key}: {type(values)} → {hvr_metrics[key]}")

                except Exception as e:
                    # 如果处理失败，记录错误但不中断
                    print(f"🔍4️⃣    ⚠️ 处理指标 {key} 失败: {e}")
                    continue

        # 将HVR指标添加到主指标字典中
        metrics.update(hvr_metrics)
        print(f"🔍4️⃣ [compute_data_metrics] 成功添加 {len(hvr_metrics)} 个HVR指标到metrics")
        print(f"🔍4️⃣ [compute_data_metrics] 最终metrics包含HVR字段: {[k for k in metrics.keys() if 'hvr' in k.lower()][:5]}...")
    else:
        print(f"🔍4️⃣ [compute_data_metrics] non_tensor_batch为空或不存在")
